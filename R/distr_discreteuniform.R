distr_discreteuniform_ui <- function(namespace) {
  ns <- NS(namespace)
  tagList(
    card(
      card_header(
...
distr_discreteuniform_server <- function(namespace) {
  ns <- NS(namespace)
  ...
  moduleServer(namespace, function(input, output, session) {
    ...
    observe({
      ...
      distr$distr <- DiscreteUniform$new(lower = input$lower, upper = input$upper)
      distr$react <- runif(1)
    }) |>
