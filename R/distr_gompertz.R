distr_gompertz_ui <- function(namespace) {
  card(
    card_header(
      "Location"
    ),
...
distr_gompertz_server <- function(namespace) {
  ns <- NS(namespace)
  ...
  moduleServer(namespace, function(input, output, session) {
    ...
    observe({
      ...
      distr$distr <- Gompertz$new(scale = input$scale)
      distr$react <- runif(1)
    }) |>
    ...
    observe({
      ...
      distr$distr <- Gompertz$new(shape = input$shape)
      distr$react <- runif(1)
    }) |>
